...
/*
function winAlert(){
	while(document.getElementById('score')
	.innerHTML>=0){
	if(document.getElementById('score').innerHTML==1){
		alert("Gratulacje!");
	}
	}
}
...
function selectImg(img) {
	if (img.className == "hidden") 
	{
		...
		if (selectedImgs.length == 2) 
		{
			...
			selectedImgs[1]);
			document.getElementById('try').innerHTML = "Dobrze!";
			tryCounter++;
			document.getElementById('tries').innerHTML = "Pozostalo " + (20-tryCounter) + " ruchow";
			}
			else 
			{
				...
				selectedImgs = [];
				document.getElementById('try').innerHTML = "Pudlo!";
				tryCounter++;
				document.getElementById('tries').innerHTML = "Pozostalo " + (20-tryCounter) + " ruchow";
		...
		if(tryCounter==20)
		{
			alert("Koniec szans, przegrales! Sprobuj jeszcze raz, powodzenia! ;)");}
